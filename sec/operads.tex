...
\begin{definition}(Operad \cite{May72})
...
\begin{definition}(Coalgebra) Let $\O$ be an operad. An $\O$\textbf{-coalgebra} is a chain complex $C$ together with chain maps
...
\begin{definition}(Comodule) Let $\O$ be an operad and $C$ an $\O$-coalgebra. A $C$\textbf{-comodule} is a chain complex $D$ together with chains maps $$\lambda:\O(j)\tensor D\to D\tensor C^{j-1}$$ satisfying the following conditions.
...
the iterates and permutations of the coproduct of the coalgebra. A $C$-comodule $D$ in the operadic sense is an $C$-bicomodule in the classical sense. \end{example}

\begin{definition}($E_\infty$-operad \cite{KM95}) \label{E infinity operad}
